...
//   Don't do these as output, it possibly could blow the pin isolators! (or maybe not...)
  ...
  ADCSRA |= 5;            //  Set prescaler to 2 or 3 (i.e. divide by 2 or 4; was 128 in standard Arduino)
...
//      Global States:
unsigned long pushed, triggered, pbholdcounter, trigcounter;
unsigned pbheld = 0;
unsigned pblongheld = 0;
...
void loop() 
{
  ...
  while (1)
  {
    ...
    if (pbholdcounter == 1000) { pbheld = 1 - pbheld; }
...
#define PHASEMAX  ((0x00000001L<<(SCALESHIFT+12))-1)
  ...
  exp_cvin  = pgm_read_word_near (exp_table + cvin) >> 5;
  exp_knob1 = pgm_read_word_near (exp_table + knob1)>> 3;
  ...
  phaseout = phaseout & 0x00000FFFL;
  dacout = pgm_read_word_near (sinetable + phaseout);     
}
...
#define KNOBSHIFT 1 
  ...
  //     supersaw oscillator
  exp_cvin  = pgm_read_word_near (exp_table + cvin);
  exp_knob1 = pgm_read_word_near (exp_table + knob1);
...
#define MAX_ARPEGGIOMODES 4
  ...
  if (arpeggiomode == 2)  //  2 is arp-pendulum
  {
    ...
    if (now_count > pitch_count) {arp_dir = -1; now_count = pitch_count; };
    if (now_count < 0) {arp_dir = +1; now_count = 0; }
    dacout = (cvin << 2) + (now_count * pitch_incr); 
  }
